...
void TcpMgr::initHandlers()
{
    ...
    _handlers.insert(ID_AUTH_FRIEND_RSP, [this](ReqId id, int len, QByteArray data) {
        ...
        rsp->SetChatDatas(chat_datas);//更新了_sthread_id
...
//Q_DECLARE_METATYPE = 让编译期知道这是一个元类型
//qRegisterMetaType = 让运行时队列连接能可靠传递它（注册到类型表）
...
{
    _tcp_thread->quit();
    //一旦事件循环退出，线程会发出 finished 信号，于是上面那行 connect 会触发 _tcp_thread->deleteLater()，最终线程对象被释放
}
