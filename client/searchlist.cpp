...
void SearchList::slot_item_clicked(QListWidgetItem *item)
{
    ...
    // 如果它确实是 ListItemBase（或其子类），转换成功；否则返回空指针，你就 return
    // static_cast：编译期转换，不做运行时类型检查，错了也可能“看起来成功”，但用起来会崩。
    // qobject_cast：运行时安全检查（基于 Qt 的元对象系统），不匹配就返回 nullptr，不会炸。
...
void SearchList::slot_user_search(std::shared_ptr<SearchInfo> si)
{
    ...
    if (si == nullptr) {
        ...
        qDebug()<<"find friend error SearchList::slot_user_search";
    }else{
