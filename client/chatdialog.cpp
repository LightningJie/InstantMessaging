...
void ChatDialog::addChatUserList()
{
    ...
    if(friend_list.empty()==false){
        for(auto friend_ele:friend_list){
            auto finder_iter=_chat_items_added.find(friend_ele->_uid);
            if(finder_iter!=_chat_items_added.end()){
                continue;
            }
            ...
            item->setSizeHint(chat_user_wid->sizeHint());//设置了尺寸
...
void ChatDialog::SetSelectChatItem(int uid)
{
    ...
    if(uid == 0){
        ui->chat_user_list->setCurrentRow(0);//设置聊天列表中的第一个项为当前选中项
...
void ChatDialog::slot_item_clicked(QListWidgetItem *item)
{
    ...
    ListItemBase *custom_base=qobject_cast<ListItemBase*>(widget);
    if(!custom_base){
        return;
    }
    auto itemType=custom_base->GetItemType();
    if(itemType==ListItemType::INVALID_ITEM||itemType==ListItemType::GROUP_TIP_ITEM){
        return;
    }
    if(itemType==ListItemType::CHAT_USER_ITEM){
        qDebug()<<"chat user item clicked";
        auto chat_wid =qobject_cast<ChatUserWid*>(custom_base);
...
void ChatDialog::slot_append_send_chat_msg(std::shared_ptr<TextChatData> msgdata)
{
    ...
    if (itemType == CHAT_USER_ITEM) {
        ...
        //设置信息
        auto user_info = con_item->GetUserInfo();
        user_info->_chat_msgs.push_back(msgdata);//这一步有用吗？有用 去掉切换聊天页面 我发送消息不显示了
        auto fi_map = UserMgr::GetInstance()->GetFriendById(_cur_chat_uid); // map里那份
        qDebug() << "map ptr=" << fi_map.get()
                 << "wid ptr=" << con_item->GetUserInfo().get();
...
{
    qDebug() << "[slot_text_chat_msg] from=" << msg->_from_uid
                 << " to=" << msg->_to_uid
                 << " msgs=" << msg->_chat_msgs.size();
    ...
    QListWidgetItem* item = new QListWidgetItem;
    item->setSizeHint(chat_user_wid->sizeHint());
    chat_user_wid->updateLastMsg(msg->_chat_msgs);//更新chatuserwid中userinfo的消息
    UserMgr::GetInstance()->AppendFriendChatMsg(msg->_from_uid,msg->_chat_msgs);
    ui->chat_user_list->insertItem(0, item);
    ui->chat_user_list->setItemWidget(item, chat_user_wid);
