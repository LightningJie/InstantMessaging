    ...
    ,m_margin(3)
{
    ...
    if(m_role == ChatRole::Self)
        m_pHLayout->setContentsMargins(m_margin, m_margin, WIDTH_SANJIAO + m_margin, m_margin);//左上右下
    else
...
void BubbleFrame::setWidget(QWidget *w)//气泡框矩形
{
...
void BubbleFrame::paintEvent(QPaintEvent *e)
{
    ...
    painter.setPen(Qt::NoPen);//不带轨迹 不要边框
    ...
    else
    {
        ...
        painter.setBrush(QBrush(bk_color));
        //画气泡
        QRect bk_rect = QRect(0, 0, this->width()-WIDTH_SANJIAO, this->height());//(0,0) = BubbleFrame 的内容区域左上角
        painter.drawRoundedRect(bk_rect,5,5);
        //画三角
        QPointF points[3] = {
